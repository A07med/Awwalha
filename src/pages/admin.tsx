...
import { adminClearRegistrations, adminCloseRound, adminPrepareRound, adminResetGames, adminRevealWinners, adminSetRegistration, adminStartTieBreak, adminResolveRound } from '../lib/api'

export function AdminPage() {
  const { state } = usePublicState({ phase: 'lobby', operator: true }, demoLobbyState)
  const [tab, setTab] = useState<'perfect' | 'look'>('perfect')
  ...
  return <PageShell className="admin-page">
    ...
    <div className="admin-grid">
      <Card className="control-card">
        <div className="card-heading"><div><p className="eyebrow">الجولة التالية</p><h2>{tab === 'perfect' ? 'الثانية المثالية' : 'أول نظرة'}</h2></div><div className="game-tabs"><button className={tab === 'perfect' ? 'active' : ''} onClick={() => setTab('perfect')}><TimerReset /></button><button className={tab === 'look' ? 'active' : ''} onClick={() => setTab('look')}><Eye /></button></div></div>
