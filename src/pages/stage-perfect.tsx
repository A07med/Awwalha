...
export function StagePerfectPage() {
  ...
  return <main className="stage-page perfect-stage" dir="rtl"><AmbientBackground /><header className="stage-header"><Wordmark /><div><StatusPill tone="gold"><Clock3 size={16} /> الثانية المثالية</StatusPill><span>ألعاب حيّة · لحظات لا تتكرر</span></div></header>
    ...
    : <section className="stage-center"><Clock3 className="stage-hero-icon" aria-hidden="true" /><h1 className="stage-game-title">الثانية المثالية</h1><p className="stage-prompt">الوقت المستهدف</p><h2 className="stage-target" dir="ltr">{((round?.targetMs ?? 6000) / 1000).toFixed(3)}<small> ثانية</small></h2><div className={'stage-timer ' + (hidden ? 'masked' : '')} dir="ltr">{countdown ? countdown : hidden ? '?.???' : Math.max(0, elapsedMs / 1000).toFixed(3)}</div><p className="stage-instruction">{hidden ? 'احسبها بنفسك' : countdown ? 'استعد' : 'الوقت يمشي الآن'}</p></section>}
