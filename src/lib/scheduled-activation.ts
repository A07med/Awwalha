...
export function scheduledInteractionActive(state: PublicEventState, clockRoundId: string | null, elapsedMs: number, participantPublicId: string) {
  ...
  if (round.parentRoundId && !state.tieEligiblePublicIds.includes(participantPublicId)) return false
